...
object Parser {
  ...
  def callExpr[_: P] = P( str | number| dict | local | func | ident )

  def number[_: P] = P( CharsWhileIn("0-9").!.map(_.toInt) ).map(Expr.Number)
...
def evaluate(expr: Expr, scope: Map[String, Value]): Value = expr match {
  ...
  case Expr.Number(i) => Value.Number(i)
  case Expr.Dict(kvs) => Value.Dict(kvs.map{case (k, v) => (k, evaluate(v, scope))})
  case Expr.Plus(left, right) =>
    val leftVal = evaluate(left, scope)
    val rightVal = evaluate(right, scope)
    (leftVal, rightVal) match {
      case (Value.Str(leftStr), Value.Str(rightStr)) => Value.Str(leftStr + rightStr)
      case (Value.Number(leftNum), Value.Number(rightNum)) => Value.Number(leftNum + rightNum)
      case (_, _) => throw new Exception("Addition operands don't match")
    }
